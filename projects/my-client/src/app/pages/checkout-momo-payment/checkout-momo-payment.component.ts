...
@Component({
  selector: 'app-checkout-momo-payment',
  standalone: true,
  imports: [RouterLink, CheckoutStepperComponent],
  templateUrl: './checkout-momo-payment.component.html',
  styleUrls: ['./checkout-momo-payment.component.css'],
})
export class CheckoutMomoPaymentComponent implements OnInit {
  ...
  qrContent = computed(() => {
    const a = this.amount;
    if (a <= 0) return '';
    if (this.orderNumber) return `AURAPC|${this.orderNumber}|${a}|Thanh toan MoMo AuraPC`;
    return `AURAPC|${a}|${Date.now()}|Thanh toan MoMo AuraPC`;
  });
